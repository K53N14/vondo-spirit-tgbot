...
- базовые: помощь, синхронизация, просмотр своих групп;
- для владельца (`OWNER_USER_IDS`) — кнопка `🛠 Админ-панель` с действиями по пользователям и группам.

Для операций, где нужны параметры (например, назначение админа или изменение прав), кнопка запускает сценарий и бот просит ввести данные в одном сообщении по подсказке.
