...
- `/apply_admins_here` — в текущем чате обновить права только тем пользователям из БД, кто уже админ: модераторам выставляются `FULL_ADMIN_RIGHTS`, остальным — `DEFAULT_ADMIN_RIGHTS`; затем применяется rank из БД (если есть).
