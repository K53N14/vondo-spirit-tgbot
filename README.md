...
Для изменения прав администратора используется inline-меню с чекбоксами (✅/⬜) по всем доступным правам, а также есть отдельная кнопка демоута (снять все права сразу).
...
- `/promote_admin <chat_id|all> <username>` — назначить пользователя из БД администратором в указанном чате или во всех активных (для OWNER_USER_IDS).
- `/set_admin_rank <chat_id|all> <username> <rank>` — задать custom title (rank) администратору в чате/группе/канале (для OWNER_USER_IDS).
