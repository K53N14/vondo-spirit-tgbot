...
- `/apply_admins_here` — в текущем чате применить права как в `/set_admin_rights` для всех участников из БД, кто состоит в чате: модераторам выставляются `FULL_ADMIN_RIGHTS`, остальным — `DEFAULT_ADMIN_RIGHTS`; затем применяется rank из БД (если есть).
